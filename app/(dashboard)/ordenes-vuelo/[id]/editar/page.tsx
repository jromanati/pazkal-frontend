...
export default function EditarOrdenVueloPage() {
  ...
  }, [mounted, canRead, formData.empresa])

  useEffect(() => {
    if (!mounted || !canRead) return
    loadOperators(formData.sucursal)
  }, [mounted, canRead, formData.sucursal])
  ...
  useEffect(() => {
    ...
    const loadOrder = async () => {
      ...
      try {
        ...
        const branchId = String(o.branch?.id ?? o.branch_id ?? '')
        let companyId = String(o.company?.id ?? o.company_id ?? '')
        if (branchId) {
          const branchRes = await BranchService.getBranch(branchId)
          if (branchRes.success && branchRes.data?.company?.id) {
            companyId = String(branchRes.data.company.id)
          }
        }
